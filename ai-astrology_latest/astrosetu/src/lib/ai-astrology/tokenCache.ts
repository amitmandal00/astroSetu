...
function cacheToken(
  ...
  try {
    ...
    if (reportType) {
      ...
      sessionStorage.removeItem("aiAstrologyReportType");
    }
    if (bundleType) {
    ...
    if (bundleReports && bundleReports.length > 0) {
...
export async function createOrReuseToken(
  ...
  if (cached) {
    ...
    const reportTypeMatches = (cached.reportType || null) === (reportType || null);
    const bundleTypeMatches = (cached.bundleType || null) === (bundleType || null);
    const cachedBundleReports = cached.bundleReports || [];
    const requestedBundleReports = bundleReports || [];
    const bundleReportsMatch =
      cachedBundleReports.length === requestedBundleReports.length &&
      cachedBundleReports.every((value, index) => value === requestedBundleReports[index]);

    if (inputMatches && reportTypeMatches && bundleTypeMatches && bundleReportsMatch) {
      ...
      // Input or report/bundle selection changed - clear cache and create new token
      console.log("[TokenCache] Input or selection changed, clearing cache and creating new token");
      clearCachedToken();
    }
  }
