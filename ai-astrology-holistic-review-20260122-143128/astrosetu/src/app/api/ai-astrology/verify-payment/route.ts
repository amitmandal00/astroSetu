...
export async function GET(req: Request) {
  ...
  try {
    ...
    // CRITICAL FIX: Handle both test_session_ (demo) and prodtest_ (prod test users)
    // Both should bypass Stripe, but prodtest_ is for production test users who want real reports
    ...
    // HARD GUARD: test/bypass must never leak in production unless explicitly enabled.
    ...
    if (isTestSession) {
      if (!enableTestBypass && process.env.NODE_ENV === "production") {
        Sentry.captureMessage("Blocked verify-payment test session in production (ENABLE_TEST_BYPASS not set)", {
          level: "warning",
          tags: { requestId, source: "verify-payment", host },
          extra: { sessionIdPrefix: sessionId.substring(0, 12) },
        });
        return NextResponse.json(
          { ok: false, error: "Test payments are disabled in production" },
          { status: 403, headers: { "X-Request-ID": requestId } }
        );
      }
      if (isProdDomain) {
        Sentry.captureMessage("verify-payment bypass used in production domain", {
          level: "warning",
          tags: { requestId, source: "verify-payment", host },
          extra: { sessionIdPrefix: sessionId.substring(0, 12) },
        });
