...
export async function POST(req: Request) {
  ...
  try {
    ...
    // HARD GUARD: test/bypass must never leak to real users in production.
    // Only enable bypass when explicitly allowed.
    ...
    const shouldBypassPayment = isDemoMode || ((isTestUser && bypassPaymentForTestUsers) || (fallbackTestUserCheck && enableTestBypass));
    if (shouldBypassPayment) {
      if (isProdDomain && !enableTestBypass) {
        Sentry.captureMessage("Blocked test bypass attempt in production domain (ENABLE_TEST_BYPASS not set)", {
          level: "warning",
          tags: { requestId, source: "create-checkout", host },
        });
      }
      if (isProdDomain && enableTestBypass) {
        Sentry.captureMessage("Test payment bypass used in production domain", {
          level: "warning",
          tags: { requestId, source: "create-checkout", host },
          extra: { isTestUser, fallbackTestUserCheck, reportType: reportType || "subscription" },
        });
      }
