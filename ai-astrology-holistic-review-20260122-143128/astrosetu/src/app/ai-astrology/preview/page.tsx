...
function PreviewContent() {
  ...
  // CRITICAL FIX (ChatGPT Feedback): Enhanced progress stages for better UX
  // Detailed stages hide variability and build trust during generation
  type LoadingStage = 
    | "verifying" 
    | "analysing"      // Analysing birth details...
    | "preparing"       // Preparing yearly themes... (for year-analysis) or Preparing insights... (for others)
    | "generating"      // Generating report content...
    | "finalising";     // Finalising insights...
  const [loadingStage, setLoadingStage] = useState<LoadingStage | null>(null); // Track loading stage for better UX
  ...
  // CRITICAL FIX (Vercel Logs Analysis): Timer persists across page refreshes via sessionStorage
  // Timer is only cleared when: 1) Navigation happens, 2) Component unmounts, 3) New generation starts
  // This prevents timer from resetting if user refreshes during/after generation
  ...
  const generateReport = useCallback(async (inputData: AIAstrologyInput, type: ReportType, currentSessionId?: string, currentPaymentIntentId?: string) => {
    ...
    setLoading(true);
    // CRITICAL FIX (ChatGPT Feedback): Enhanced progress stages for better UX
    // Start with "verifying" stage, then progress through stages
    ...
    try {
      ...
      // Timeout budget:
      // - Heavy reports are processed asynchronously in the background (POST returns 202 quickly).
      //   Polling must allow several minutes for delivery.
      // - Non-heavy reports remain synchronous and should complete quickly.
      const isHeavyAsync = type === "full-life" || type === "career-money" || type === "major-life-phase" || type === "decision-support";
      const isFreeReport = type === "life-summary";
      const clientTimeout = isHeavyAsync ? 12 * 60 * 1000 : (isFreeReport ? 90_000 : 120_000);
      ...
      if (response.ok && response.data?.status === "processing" && response.data?.reportId) {
        ...
        // CRITICAL FIX (Vercel Logs Analysis): Reduce polling interval for faster updates
        // Reduced from 3s to 2s to improve perceived performance
        const pollInterval = 2000; // 2 seconds (was 3s)
        ...
        const pollForReport = async (): Promise<void> => {
          ...
          // CRITICAL FIX (ChatGPT Feedback): Update progress stages during polling
          // Show "generating" for most reports, "finalising" when close to completion
          const progressRatio = pollAttempts / maxPollAttempts;
          if (progressRatio > 0.7) {
            // More than 70% through polling - show "finalising"
            setLoadingStage("finalising");
          } else if (progressRatio > 0.3) {
            // More than 30% through polling - show "generating"
            const isYearAnalysis = type === "year-analysis";
            setLoadingStage(isYearAnalysis ? "preparing" : "generating");
          }
          ...
          try {
            ...
            if (statusData.ok && statusData.data) {
              if (statusData.data.status === "completed" || statusData.data.status === "DELIVERED") {
                ...
                setLoading(false);
                setLoadingStage(null);
                // CRITICAL FIX (Vercel Logs Analysis): Don't clear timer immediately on completion
                // Keep timer visible until user navigates away or component unmounts
                // This prevents timer from resetting if user refreshes page during/after generation
                // Timer will be cleared on component unmount or navigation
                isGeneratingRef.current = false;
                hasAutoGeneratedRef.current = false;
                activeAttemptKeyRef.current = null; // Clear attempt key on completion
                // NOTE: loadingStartTimeRef and loadingStartTime are NOT cleared here
                // They will be cleared on component unmount or when new generation starts
                ...
                // CRITICAL FIX (Vercel Logs Analysis): Clear timer only when navigating away
                // Timer will persist until navigation, allowing user to see final elapsed time
                // If user refreshes before navigation, timer will be restored from sessionStorage
                ...
                if (statusData.data.redirectUrl) {
                  ...
                  // Clear timer just before navigation (user won't see it anyway)
                  loadingStartTimeRef.current = null;
                  setLoadingStartTime(null);
      ...
      // CRITICAL FIX (Priority 1): Handle "needs_regeneration" status
      // Show banner with regenerate button instead of showing broken report
      const responseData = response.data as typeof response.data & { status?: "completed" | "processing" | "pending" | "failed" | "needs_regeneration"; error?: string; errorCode?: string };
      if (responseData && (responseData.status as string) === "needs_regeneration") {
        console.log("[CLIENT] Report needs regeneration", {
          reportId: responseData.reportId,
          reportType: responseData.reportType,
          error: responseData.error,
        });
        setNeedsRegeneration(true);
        setRegenerationError(responseData.error || "Report generation failed. Please try again.");
        setLoading(false);
        setLoadingStage(null);
        isGeneratingRef.current = false;
        hasAutoGeneratedRef.current = false;
        return; // Exit - don't show broken report
      }
      ...
      if (isTimeoutError) {
        ...
        const isHeavyAsync = type === "full-life" || type === "career-money" || type === "major-life-phase" || type === "decision-support";
        const calculatedTimeout = isHeavyAsync ? 12 * 60 * 1000 : (type === "life-summary" ? 90_000 : 120_000);
        const timeoutMessage = isHeavyAsync
          ? `Report generation is taking longer than expected (>${Math.round(calculatedTimeout / 1000)}s). Please keep this tab open; the report will appear automatically when ready.`
          : `Report generation timed out after ${Math.round(calculatedTimeout / 1000)} seconds. Please try again.`;
  ...
  const generateBundleReports = useCallback(async (inputData: AIAstrologyInput, reports: ReportType[], currentSessionId?: string, currentPaymentIntentId?: string) => {
    ...
    try {
      ...
      // Individual timeout for each report in a bundle.
      // Heavy reports are async now and can take several minutes to complete.
      const INDIVIDUAL_REPORT_TIMEOUT = 12 * 60 * 1000; // 12 minutes

      const pollBundleReport = async (reportId: string, signal: AbortSignal): Promise<any> => {
        const pollInterval = 2000;
        const maxAttempts = Math.floor(INDIVIDUAL_REPORT_TIMEOUT / pollInterval);
        let attempts = 0;
        while (attempts < maxAttempts) {
          attempts++;
          const res = await fetch(`/api/ai-astrology/generate-report?reportId=${encodeURIComponent(reportId)}`, {
            method: "GET",
          ...
          if (res.ok) {
            ...
            if (statusData?.ok && statusData?.data && (statusData.data.status === "completed" || statusData.data.status === "DELIVERED")) {
              return statusData;
            }
        ...
        return { ok: false, error: "Bundle report polling timed out" };
      };
      ...
      const generateSingleReport = async (reportType: ReportType) => {
        ...
        try {
          ...
          // If generation is async, we'll get processing status - poll until ready
          if (data.ok && data.data?.status === "processing" && data.data?.reportId) {
            const statusData = await pollBundleReport(data.data.reportId, abortController.signal);
            if (statusData?.ok && statusData?.data?.content) {
              console.log(`[BUNDLE] Successfully generated (after polling): ${reportName}`);
              updateProgress(reportType, true);
              return { reportType, content: statusData.data.content, success: true };
            }
            const err = statusData?.error || statusData?.data?.error || "Failed to generate report (processing timeout)";
            updateProgress(reportType, false);
            return { reportType, content: null, success: false, error: err };
          }
          ...
          <CardContent className="p-12 text-center">
            ...
            <h2 className="text-2xl font-bold mb-2 text-slate-900">
              {isVerifying 
                ...
                ? `Analysing Your ${getReportName(reportType || "life-summary")}...`
                : loadingStage === "preparing"
                ? `Preparing Your ${getReportName(reportType || "life-summary")}...`
                : loadingStage === "generating"
                ? `Generating Your ${getReportName(reportType || "life-summary")}...`
                : loadingStage === "finalising"
                ? `Finalising Your ${getReportName(reportType || "life-summary")}...`
                : `Generating Your ${getReportName(reportType || "life-summary")}...`}
            </h2>
                ...
                <div className="mb-6">
                  ...
                  <div className="mb-4 text-center">
                    <p className="text-sm text-slate-600 mb-2">
                      {/* CRITICAL FIX (ChatGPT Feedback): Show detailed progress stages */}
                      {isVerifying
                        ? "💳 Verifying payment..."
                        : loadingStage === "analysing"
                        ? "🔍 Analysing birth details and planetary positions..."
                        : loadingStage === "preparing"
                        ? reportType === "year-analysis"
                          ? "📅 Preparing yearly themes and monthly insights..."
                          : "✨ Preparing personalized insights..."
                        : loadingStage === "generating"
                        ? "🤖 Generating report content..."
                        : loadingStage === "finalising"
                        ? "🎯 Finalising insights and recommendations..."
                        : elapsedTime < 30 
  ...
  const renderSingleReportContent = (content: ReportContent | null, type: ReportType) => {
    ...
    return (
      <>
                {/* Needs Regeneration Banner - Priority 1 Fix */}
                {needsRegeneration && (
                    ...
                    <div className="flex items-start gap-3">
                      ...
                      <div className="flex-1">
                        ...
                        <p className="text-sm text-red-800 mb-3">
                          {regenerationError || "We couldn't generate the full report. The content contained placeholder text that couldn't be fixed automatically."}
                        </p>
                        <Button
                          onClick={async () => {
                            ...
                            setError(null);
                            setLoading(true);
                            // Trigger regeneration by reloading the page with auto_generate=true
                            // This is simpler than trying to reconstruct the input data
                            const currentUrl = new URL(window.location.href);
                            currentUrl.searchParams.set("auto_generate", "true");
