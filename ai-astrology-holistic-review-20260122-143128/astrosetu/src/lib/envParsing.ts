...
export function calculateReportMode(params: {
  ...
  // Rule 4: FORCE_REAL_REPORTS env var (global override - takes priority over test sessions)
  // This allows forcing real reports even for test sessions
  if (forceRealReportsEnv) {
    return { shouldUseRealMode: true, mockMode: false };
  }

  // Rule 5: For test sessions (test_session_*), use mock unless ALLOW_REAL_FOR_TEST_SESSIONS=true
  if (isTestSession) {
