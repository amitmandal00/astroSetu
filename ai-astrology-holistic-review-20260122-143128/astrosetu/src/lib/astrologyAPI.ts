...
async function executeProkeralaRequest(endpoint: string, params: Record<string, any>, retries: number = 2, method: "GET" | "POST" = "POST", skipCache: boolean = false): Promise<any> {
  // Fast-path: skip Dosha endpoint calls when it's known-broken for a cooldown period.
  // This avoids wasting time and reduces cascading latency in report generation.
  if (isDoshaCircuitOpen(endpoint)) {
    throw new Error("PROKERALA_DOSHA_CIRCUIT_OPEN");
  }
  ...
  for (let attempt = 0; attempt <= retries; attempt++) {
    try {
      ...
      lastError = error;
      maybeTripDoshaCircuit(endpoint, error);
...
// --- Dosha endpoint circuit breaker (best-effort, in-memory) ---
let doshaCircuitOpenUntilMs = 0;
let doshaCircuitReason: string | null = null;

function isDoshaCircuitOpen(endpoint: string): boolean {
  if (!(endpoint === "/dosha" || endpoint.includes("/dosha"))) return false;
  return Date.now() < doshaCircuitOpenUntilMs;
}

function maybeTripDoshaCircuit(endpoint: string, error: any) {
  if (!(endpoint === "/dosha" || endpoint.includes("/dosha"))) return;
  const msg = String(error?.message || "");
  const lower = msg.toLowerCase();
  const is404 = msg.includes("404") || lower.includes("no route found") || lower.includes("endpoint_not_available");
  const isCredits = lower.includes("credit exhausted") || lower.includes("insufficient credit") || lower.includes("prokerala_credit_exhausted");

  if (!is404 && !isCredits) return;

  // 404 / missing route is likely persistent → longer cooldown.
  // Credit exhaustion can be transient → shorter cooldown.
  const durationMs = is404 ? 6 * 60 * 60 * 1000 : 15 * 60 * 1000;
  const until = Date.now() + durationMs;
  if (until <= doshaCircuitOpenUntilMs) return;

  doshaCircuitOpenUntilMs = until;
  doshaCircuitReason = msg;
  console.warn("[AstroSetu] DOSHA circuit breaker opened", {
    until: new Date(until).toISOString(),
    durationMs,
    reason: doshaCircuitReason?.substring(0, 200),
  });
}
