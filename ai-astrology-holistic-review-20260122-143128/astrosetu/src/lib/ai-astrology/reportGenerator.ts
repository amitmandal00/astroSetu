...
import { bumpMetric, getAIJobContext } from "./aiJobContext";
 ...
 * CRITICAL FIX (ChatGPT Feedback): Export to allow route to check isDegradedInput
 */
export async function getKundliWithCache(input: AIAstrologyInput): Promise<{
  kundli: KundliResult & { chart?: KundliChart };
  kundliTime: number;
  cacheKey: string;
  isDegradedInput?: boolean; // CRITICAL: Flag indicating Prokerala exhaustion/fallback data
}> {
  ...
  if (cachedKundli) {
    ...
    // Cached data is from previous successful Prokerala call (not degraded)
    return {
      kundli: cachedKundli,
      kundliTime: 0,
      cacheKey,
      isDegradedInput: false,
    };
  }
  ...
  let isDegradedInput = false;
  
  try {
    bumpMetric("prokeralaCalls", 1);
    ...
    return {
      ...
      kundliTime,
      cacheKey,
      isDegradedInput: false, // Real Prokerala data
    };
  } catch (error: any) {
    // CRITICAL FIX (ChatGPT Feedback): Detect Prokerala exhaustion and mark as degraded
    ...
    if (isCreditError) {
      // CRITICAL: Mark as degraded input - this will be passed to AI to handle explicitly
      isDegradedInput = true;
      console.warn("[KundliCache] ⚠️ PROKERALA_CREDIT_EXHAUSTED - Using fallback data (DEGRADED INPUT)", {
        cacheKey: cacheKey.substring(0, 20) + "...",
        note: "AI will be informed of degraded input and must generate quality content despite limitations",
      });
    } else {
      console.warn("[KundliCache] Prokerala API error, falling back to mock data. Error:", errorMessage.substring(0, 200));
    }
    
    // Fallback to mock/engine-generated data
    console.log("[KundliCache] Generating fallback kundli data...");
    const { getKundli: getKundliFallback } = await import("../astrologyAPI");
    const kundliResult = await getKundliFallback({
      name: input.name,
      dob: input.dob,
      tob: input.tob,
      place: input.place,
    ...
    // CRITICAL: Don't cache degraded data - we want to retry Prokerala on next request
    // Only cache if it's a non-credit error (temporary issue)
    if (!isCreditError) {
      cacheKundli(cacheKey, kundliResult);
    }
    ...
    return {
      ...
      kundliTime,
      cacheKey,
      isDegradedInput, // TRUE if Prokerala credit exhausted, FALSE if other error
    };
  }
}
...
export async function generateAIContent(
  ...
  if (OPENAI_API_KEY) {
    // Sync mode must stay fast and deterministic to avoid serverless timeouts.
    // Background mode can afford retries for better reliability.
    const mode = getAIJobContext()?.mode ?? "sync";
    const maxRetries = mode === "background" ? 2 : 0;
    return generateWithOpenAI(prompt, 0, maxRetries, reportType, sessionKey, input);
...
async function generateWithOpenAI(
  ...
  // CRITICAL FIX (Vercel Logs Analysis): Increased tokens to fix word count issues
  // Logs show reports 30-40% below minimum (496-965 words vs 800-1000 required)
  // Increased budgets to ensure adequate word count generation
  const maxTokens = reportType === "full-life" ? 2800 :  // Was 2400
                    reportType === "major-life-phase" ? 3000 :  // Was 2600 → Increased for reliability
                    reportType === "year-analysis" ? 3000 :  // Was 2600 → Increased for 0-section fix
                    reportType === "career-money" ? 2200 :  // Was 1800 → Increased for word count
                    reportType === "marriage-timing" ? 2200 :  // Was 1800 → Increased for word count
                    isFreeReport ? 1400 : 1800; // Free: 1400, Other paid: 1800
  
  // Use per-call timeouts that depend on execution mode.
  // - sync: must be < route watchdog (fast fail, UI retries / async job can take over)
  // - background: can be longer to make heavy reports feasible
  const mode = getAIJobContext()?.mode ?? "sync";
  const clientTimeoutMs = mode === "background" ? 90000 : 25000;
  ...
  try {
    ...
    if (response.status === 429 || errorData?.error?.code === "rate_limit_exceeded") {
      // CRITICAL FIX: Check if we're already past the timeout before retrying
      // If the signal is aborted, don't retry (route timeout has triggered)
      if (controller.signal.aborted) {
        throw new Error("Request aborted due to timeout");
      }
      
      if (retryCount < maxRetries) {
        bumpMetric("openaiRetries", 1);
        ...
        let waitTime = isFreeReportType ? 2000 : 3000; // 2s for free, 3s for paid (reduced further for timeout compliance)
        ...
        // Add jitter (random 0-1 seconds, reduced further) to avoid thundering herd
        const jitter = Math.random() * 1000;
        // CRITICAL FIX: Cap total wait at 5 seconds to respect route timeout (30s)
        // With 25s client timeout, we need to keep retries very short
        const totalWait = Math.min(waitTime + jitter, 5000); // Cap at 5 seconds total
    ...
    if (fetchError.name === 'AbortError' || controller.signal.aborted) {
      ...
      throw new Error(`OpenAI API request timed out after ${Math.round(clientTimeoutMs / 1000)} seconds. Please try again.`);
    }
...
function parseAIResponse(response: string, reportType: ReportType, reportId?: string): ReportContent {
  // CRITICAL FIX (Vercel Logs Analysis): Add logging for year-analysis parsing issues
  // Logs show year-analysis sometimes returns 0 sections, indicating parsing failures
  const parseStartTime = Date.now();
  const originalResponseLength = response?.length || 0;
  ...
  // CRITICAL FIX (Vercel Logs Analysis): Add JSON extraction fallback
  // AI may return JSON in markdown code blocks - try to extract it first
  // This improves parsing reliability for year-analysis and other reports
  try {
    ...
    if (jsonMatch && jsonMatch[1]) {
      try {
        ...
        if (jsonContent && jsonContent.sections && Array.isArray(jsonContent.sections) && jsonContent.sections.length > 0) {
          console.log("[parseAIResponse] Successfully extracted JSON from markdown code blocks", {
            reportType,
            sectionsCount: jsonContent.sections.length,
            hasTitle: !!jsonContent.title,
          });
          // Use JSON sections if available
          return {
            title: jsonContent.title || getReportTitle(reportType),
            sections: jsonContent.sections,
            summary: jsonContent.summary || extractSummary(response),
            executiveSummary: jsonContent.executiveSummary,
            generatedAt: new Date().toISOString(),
          };
        }
      } catch (jsonParseError) {
        // JSON parsing failed, fall through to markdown parsing
        console.warn("[parseAIResponse] JSON extraction failed, falling back to markdown parsing", {
          reportType,
    ...
    // Try to find JSON object directly in response (without code blocks)
    const directJsonMatch = response.match(/\{[\s\S]*"sections"[\s\S]*\}/);
    if (directJsonMatch && directJsonMatch[0]) {
      try {
        const jsonContent = JSON.parse(directJsonMatch[0]);
        if (jsonContent && jsonContent.sections && Array.isArray(jsonContent.sections) && jsonContent.sections.length > 0) {
          console.log("[parseAIResponse] Successfully extracted JSON directly from response", {
            reportType,
            sectionsCount: jsonContent.sections.length,
            hasTitle: !!jsonContent.title,
          });
    ...
    console.warn("[parseAIResponse] JSON extraction attempt failed, using markdown parsing", {
      reportType,
  ...
  for (const line of lines) {
    ...
    // CRITICAL FIX (Vercel Logs Analysis): Improve year-analysis section title matching
    // Use flexible matching - accept partial matches and variations
    if (yearAnalysisTitleSet) {
      // Check exact match first
      if (yearAnalysisTitleSet.has(normalizedTitle)) {
        if (currentSection) sections.push(currentSection);
        currentSection = { title: trimmed.replace(/:$/, "").trim(), content: "" };
        continue;
      }
      
      // Check for partial matches (title contains expected keyword or vice versa)
      const hasExpectedKeyword = Array.from(yearAnalysisTitleSet).some(expected => {
        // Check if normalized title contains expected keyword
        if (normalizedTitle.includes(expected) || expected.includes(normalizedTitle)) {
          return true;
        }
        // Check for common variations (e.g., "year strategy" vs "yearly strategy")
        const normalizedVariations = [
          normalizedTitle.replace(/ly\s+/g, " "), // "yearly" -> "year"
          normalizedTitle.replace(/\s+and\s+/g, " "), // "year and strategy" -> "year strategy"
          normalizedTitle.replace(/\s+/g, ""), // Remove spaces for comparison
        ];
        const expectedVariations = [
          expected.replace(/ly\s+/g, " "),
          expected.replace(/\s+and\s+/g, " "),
          expected.replace(/\s+/g, ""),
        ];
        return normalizedVariations.some(nv => expectedVariations.some(ev => nv.includes(ev) || ev.includes(nv)));
      });
      
      if (hasExpectedKeyword) {
        if (currentSection) sections.push(currentSection);
        // Use the original trimmed line as title (preserve formatting)
        currentSection = { title: trimmed.replace(/:$/, "").trim(), content: "" };
        continue;
      }
    }
  ...
  // CRITICAL FIX (Vercel Logs Analysis): Add alternative parsing strategies if markdown parsing fails
  ...
  if (reportType === "year-analysis") {
    ...
    // CRITICAL FIX (Vercel Logs Analysis): If no expected sections found, try alternative parsing strategies
    // This handles cases where AI response format doesn't match expected markdown structure
    if (!hasExpectedSections && meaningfulSections.length === 0) {
      console.warn("[parseAIResponse] Year-analysis parsing found no expected sections, trying alternative strategies", {
        reportType,
      ...
      // Alternative Strategy 1: Look for section headers with different formats
      // Try to find sections using regex patterns for common header formats
      const headerPatterns = [
        /^(?:#{1,3}\s+)?(?:Year Strategy|Year Theme|Quarter|Best Periods|Low-Return|What to Do|Year-End|Decision Anchor|Confidence Level)[:]*\s*$/im,
        /^(?:#{1,3}\s+)?(?:YEAR STRATEGY|YEAR THEME|QUARTER|BEST PERIODS)[:]*\s*$/im,
      ];
      
      // Alternative Strategy 2: Extract sections from numbered lists with bold titles
      const numberedListPattern = /^\d+\.\s*\*\*(.+?)\*\*[:]*\s*$/im;
      
      // Alternative Strategy 3: Extract sections from bullet points with bold titles
      const bulletBoldPattern = /^[-*]\s*\*\*(.+?)\*\*[:]*\s*$/im;
      
      // Try these patterns on the response
      let alternativeSections: ReportContent["sections"] = [];
      let currentAltSection: { title: string; content: string } | null = null;
      ...
      for (let i = 0; i < lines.length; i++) {
        ...
        for (const pattern of headerPatterns) {
          ...
          if (match) {
            if (currentAltSection) alternativeSections.push(currentAltSection);
            const title = trimmed.replace(/^#{1,3}\s+/, "").replace(/:$/, "").trim();
            if (title.length > 0 && title.length < 100) { // Reasonable title length
              currentAltSection = { title, content: "" };
              break;
            }
          }
        }
        
        // Check numbered list pattern
        const numberedMatch = trimmed.match(numberedListPattern);
        if (numberedMatch && numberedMatch[1]) {
          if (currentAltSection) alternativeSections.push(currentAltSection);
          currentAltSection = { title: numberedMatch[1].trim(), content: "" };
        }
        
        // Check bullet bold pattern
        const bulletMatch = trimmed.match(bulletBoldPattern);
        if (bulletMatch && bulletMatch[1]) {
          if (currentAltSection) alternativeSections.push(currentAltSection);
          currentAltSection = { title: bulletMatch[1].trim(), content: "" };
        }
        
        // If we have a current section and this line is content (not a header), add it
        if (currentAltSection && !trimmed.match(/^#{1,3}\s+|^\d+\.\s+|^[-*]\s+/)) {
          currentAltSection.content += (currentAltSection.content ? "\n" : "") + trimmed;
        }
      }
      
      // Add last alternative section
      if (currentAltSection) {
        alternativeSections.push(currentAltSection);
      }
      
      // If alternative parsing found sections with content, use them
      const meaningfulAltSections = alternativeSections.filter(s => 
        s.content && s.content.trim().length > 20
      );
      
      if (meaningfulAltSections.length > 0) {
        console.log("[parseAIResponse] Alternative parsing strategies found sections for year-analysis", {
          reportType,
          alternativeSectionsCount: meaningfulAltSections.length,
          sectionTitles: meaningfulAltSections.map(s => s.title),
        });
        // Replace sections with alternative sections
        sections.length = 0;
        sections.push(...meaningfulAltSections);
        // Update meaningfulSections to use alternative sections
        // This will prevent fallback from being triggered
        shouldUseFallbackForYearAnalysis = false;
      } else {
        // Alternative parsing also failed, use fallback
        shouldUseFallbackForYearAnalysis = true;
      }
    } else if (!hasExpectedSections) {
      // Has sections but not expected ones - still use fallback for year-analysis
      shouldUseFallbackForYearAnalysis = true;
    }
    
    // CRITICAL FIX: Only check content length if we haven't already determined fallback is needed
    // If alternative parsing succeeded, don't override that decision
    ...
    if (!shouldUseFallbackForYearAnalysis) {
      ...
      totalContentLength = meaningfulSections.reduce((sum, s) => {
        return sum + (s.content?.length || 0) + (s.bullets?.join(" ").length || 0);
      }, 0);
      
      // If total content is too short, use fallback
      if (totalContentLength < 500) {
        shouldUseFallbackForYearAnalysis = true;
      }
    } else {
      // Calculate for logging purposes even if fallback is already determined
      totalContentLength = meaningfulSections.reduce((sum, s) => {
        return sum + (s.content?.length || 0) + (s.bullets?.join(" ").length || 0);
      }, 0);
    }
  ...
  // CRITICAL FIX (Vercel Logs Analysis): Recalculate meaningfulSections if alternative parsing succeeded
  // Alternative parsing may have added sections that weren't in the original meaningfulSections
  const finalMeaningfulSections = shouldUseFallbackForYearAnalysis === false && sections.length > 0
  ...
  if (finalMeaningfulSections.length === 0 || !response || response.trim().length === 0 || shouldUseFallbackForYearAnalysis) {
    console.warn("[parseAIResponse] No meaningful sections parsed or empty response - creating fallback report", {
      ...
      meaningfulSections: finalMeaningfulSections.length,
    ...
    sections.length = 0;
    sections.push(...finalMeaningfulSections);
  }
  ...
  // CRITICAL FIX (Vercel Logs Analysis): Log parsing results for year-analysis debugging
  const parseTime = Date.now() - parseStartTime;
  const finalSectionsCount = reportWithFallbacks.sections?.length || 0;
  
  // CRITICAL FIX: Calculate final meaningful sections count for logging
  // Use finalMeaningfulSections if available (after fallback check), otherwise use meaningfulSections
  const finalMeaningfulCount = typeof finalMeaningfulSections !== 'undefined' 
    ? finalMeaningfulSections.length 
    : meaningfulSections.length;
  
  if (reportType === "year-analysis") {
    console.log("[parseAIResponse] Year-analysis parsing complete", {
      reportType,
      originalResponseLength,
      parsedSectionsCount: sections.length,
      meaningfulSectionsCount: finalMeaningfulCount,
      finalSectionsCount,
      usedFallback: shouldUseFallbackForYearAnalysis || finalMeaningfulCount === 0,
      parseTimeMs: parseTime,
      sectionTitles: reportWithFallbacks.sections?.map(s => s.title) || [],
    });
    
    // CRITICAL FIX (Vercel Logs Analysis): Add detailed logging when parsing fails
    // If still 0 sections after fallback, log comprehensive debugging information
    if (finalSectionsCount === 0) {
      console.error("[parseAIResponse] CRITICAL: Year-analysis has 0 sections after fallback!", {
        reportType,
        originalResponseLength,
        responsePreview: response?.substring(0, 2000), // Log first 2000 chars
        responseEnd: response?.substring(Math.max(0, response.length - 500)), // Log last 500 chars
        parseTimeMs: parseTime,
        linesCount: lines.length,
        firstLines: lines.slice(0, 20), // First 20 lines for debugging
        lastLines: lines.slice(-10), // Last 10 lines for debugging
        sectionTitlesFound: sections.map(s => s.title), // What sections were found before fallback
      });
    } else if (sections.length === 0 && finalSectionsCount > 0) {
      // Parsing returned 0 sections but fallback added sections - log for analysis
      console.warn("[parseAIResponse] Year-analysis parsing returned 0 sections, fallback added sections", {
        reportType,
        originalResponseLength,
        finalSectionsCount,
        responsePreview: response?.substring(0, 1000), // Log first 1000 chars
        parseTimeMs: parseTime,
        linesCount: lines.length,
        firstLines: lines.slice(0, 15), // First 15 lines
      });
    }
  }
  ...
  if (paidReportTypes.includes(reportType) && sections.length < minSectionsForPaid) {
    ...
    if (paidReportTypes.includes(reportType)) {
      ...
      if (currentWordCount < minWords) {
        ...
        // CRITICAL FIX: Add more specific sections based on report type instead of generic "Comprehensive Analysis"
        for (let i = 0; i < additionalSectionsNeeded && i < 3; i++) {
          if (!existingTitles.has(`additional insights ${i + 1}`) && !existingTitles.has(`comprehensive analysis ${i + 1}`)) {
            // Use report-type-specific section titles and content instead of generic
            let sectionTitle: string;
            let sectionContent: string;
            
            if (reportType === "major-life-phase") {
              const phaseSections = [
                { title: "Long-Term Strategic Planning", content: "This phase requires strategic planning that extends beyond immediate concerns. Consider your 5-year goals, key milestones, and areas for development. Focus on building sustainable foundations that will support long-term growth. The planetary influences during this phase create opportunities for significant life changes, but these changes require careful planning and preparation. Understanding the long-term trajectory helps you make decisions that align with your overall life path." },
                { title: "Personal Growth and Development", content: "Personal growth is a key theme during this phase. Opportunities for skill development, education, and self-improvement align with favorable planetary influences. Focus on areas where you want to grow and develop, whether in career, relationships, or personal interests. The phase's energy supports learning, adaptation, and personal transformation. Investing in your own development during this time creates a foundation for future success." },
                { title: "Relationship and Social Dynamics", content: "This phase brings changes in relationships and social dynamics. New connections may form, existing relationships may deepen, and some relationships may evolve or change. Understanding the astrological influences on your 7th house (partnerships) and 11th house (friendships) helps you navigate these changes effectively. Focus on building meaningful connections and maintaining healthy relationship boundaries." },
              ];
              const sectionIndex = Math.min(i, phaseSections.length - 1);
              sectionTitle = phaseSections[sectionIndex].title;
              sectionContent = phaseSections[sectionIndex].content;
            } else if (reportType === "marriage-timing") {
              const marriageSections = [
                { title: "Relationship Preparation", content: "Preparation for marriage involves both practical and emotional readiness. Focus on building communication skills, understanding, and relationship maturity. The astrological timing for marriage aligns with favorable planetary influences, but personal readiness is equally important. Use this time to prepare yourself for a committed partnership." },
                { title: "Compatibility Considerations", content: "Astrological compatibility involves analyzing how your birth chart aligns with potential partners' charts. Certain planetary placements and aspects indicate natural compatibility and relationship harmony. Understanding these factors helps you identify partners who align with your astrological profile and relationship needs." },
              ];
              const sectionIndex = Math.min(i, marriageSections.length - 1);
              sectionTitle = marriageSections[sectionIndex].title;
              sectionContent = marriageSections[sectionIndex].content;
            } else {
              // Generic fallback for other report types
              sectionTitle = `Comprehensive Analysis - Section ${sections.length + 1}`;
              sectionContent = "This section provides additional astrological insights based on your birth chart analysis. The interplay between your natal chart patterns, current Dasha period, and planetary transits creates unique opportunities and challenges. Understanding these influences helps you navigate your life path more effectively and make decisions aligned with favorable astrological timing. Regular reflection on these insights helps you stay aligned with evolving opportunities and challenges throughout your journey.";
            }
    ...
    // CRITICAL FIX: Use report-type-specific section titles instead of generic "Additional Insights"
    while (sections.length < minSectionsForPaid) {
      ...
      if (reportType === "major-life-phase") {
        ...
        const sectionIndex = sections.length - minSectionsForPaid;
        sectionTitle = remainingSections[Math.abs(sectionIndex) % remainingSections.length] || `Strategic Insights - Section ${sections.length + 1}`;
        sectionContent = "This phase requires strategic thinking and long-term planning. Focus on aligning your actions with favorable astrological timing and building sustainable foundations for future growth. The planetary influences during this phase create opportunities for significant life changes, but these changes require careful planning and preparation.";
      } else if (reportType === "marriage-timing") {
        const remainingSections = [
          "Relationship Timing and Preparation",
          "Partnership Compatibility Factors",
          "Marriage Readiness Indicators",
        ];
        const sectionIndex = sections.length - minSectionsForPaid;
        sectionTitle = remainingSections[Math.abs(sectionIndex) % remainingSections.length] || `Marriage Insights - Section ${sections.length + 1}`;
        sectionContent = "Marriage timing involves both astrological and personal readiness factors. Understanding the favorable timing windows helps you plan for marriage at optimal times. Focus on building relationship skills and understanding during preparation phases.";
      } else {
        // Generic fallback for other report types
        sectionTitle = `Additional Insights - Section ${sections.length + 1}`;
        sectionContent = "This section contains additional astrological insights based on your birth chart analysis. For comprehensive guidance, consider reviewing all sections of this report together.";
      }
...
/**
 * CRITICAL FIX (ChatGPT Feedback): Add degradation awareness to prompt when Prokerala exhausted
 * This ensures AI knows about data quality limitations and generates quality content despite them
 */
function addDegradationAwarenessToPrompt(prompt: string, input: AIAstrologyInput): string {
  const degradationNote = `

CRITICAL: DEGRADED INPUT DATA DETECTED
- Prokerala API credit exhausted - using fallback/calculated data instead of real API data
- Planetary positions and calculations are based on standard algorithms, not verified API data
- You MUST still generate HIGH-QUALITY, PERSONALIZED content despite this limitation
- DO NOT use placeholder text or generic content
- DO NOT mention data quality issues in the report
- Generate complete, detailed content as if using real API data
- Focus on providing value despite data source limitations
- Minimum word counts and quality standards still apply
- Personalize all content to ${input.name} (born ${input.dob})`;
  return prompt + degradationNote;
}
...
export async function generateLifeSummaryReport(input: AIAstrologyInput, sessionKey?: string): Promise<ReportContent> {
  ...
  try {
    ...
    const { kundli: kundliResult, kundliTime, isDegradedInput } = await getKundliWithCache(input);
    console.log(`[generateLifeSummaryReport] Kundli data ready (${kundliTime}ms), isDegradedInput: ${isDegradedInput || false}, extracting planetary data...`);
    ...
    // Generate prompt
    let prompt = generateLifeSummaryPrompt(
      {
        name: input.name,
        dob: input.dob,
        tob: input.tob,
    ...
    // CRITICAL FIX (ChatGPT Feedback): Add degradation awareness if Prokerala exhausted
    if (isDegradedInput) {
      prompt = addDegradationAwarenessToPrompt(prompt, input);
      console.warn(`[generateLifeSummaryReport] ⚠️ DEGRADED INPUT - Added degradation awareness to prompt`);
    }

    // Generate AI content (pass reportType for proper retry handling and logging)
    console.log(`[generateLifeSummaryReport] Generating AI content${isDegradedInput ? ' (DEGRADED INPUT MODE)' : ''}...`);
...
export async function generateMarriageTimingReport(input: AIAstrologyInput, sessionKey?: string): Promise<ReportContent> {
  ...
  try {
    ...
    // Get astrology data with caching (optimized)
    const { kundli: kundliResult, kundliTime, cacheKey, isDegradedInput } = await getKundliWithCache(input);
    console.log(`[generateMarriageTimingReport] Kundli data ready (${kundliTime}ms), isDegradedInput: ${isDegradedInput || false}`);
    ...
    // Generate prompt
    let prompt = generateMarriageTimingPrompt(
      {
        name: input.name,
        dob: input.dob,
        tob: input.tob,
    ...
    if (isDegradedInput) {
      prompt = addDegradationAwarenessToPrompt(prompt, input);
      console.warn(`[generateMarriageTimingReport] ⚠️ DEGRADED INPUT - Added degradation awareness to prompt`);
    }

    // Generate AI content (pass reportType for proper retry handling and logging)
    console.log(`[generateMarriageTimingReport] Generating AI content${isDegradedInput ? ' (DEGRADED INPUT MODE)' : ''}...`);
    const aiStartTime = Date.now();
...
export async function generateCareerMoneyReport(input: AIAstrologyInput, sessionKey?: string): Promise<ReportContent> {
  ...
  try {
    ...
    // Get astrology data with caching (optimized)
    const { kundli: kundliResult, kundliTime, isDegradedInput } = await getKundliWithCache(input);
    console.log(`[generateCareerMoneyReport] Kundli data ready (${kundliTime}ms), isDegradedInput: ${isDegradedInput || false}, extracting planetary data...`);
    ...
    // Generate prompt
    let prompt = generateCareerMoneyPrompt(
      {
        name: input.name,
        dob: input.dob,
        tob: input.tob,
    ...
    if (isDegradedInput) {
      prompt = addDegradationAwarenessToPrompt(prompt, input);
      console.warn(`[generateCareerMoneyReport] ⚠️ DEGRADED INPUT - Added degradation awareness to prompt`);
    }

    // Generate AI content (pass reportType for proper retry handling and logging)
    console.log(`[generateCareerMoneyReport] Generating AI content${isDegradedInput ? ' (DEGRADED INPUT MODE)' : ''}...`);
    const aiStartTime = Date.now();
...
export async function generateFullLifeReport(input: AIAstrologyInput, sessionKey?: string): Promise<ReportContent> {
  ...
  try {
    ...
    // Get astrology data with caching (optimized)
    const { kundli: kundliResult, kundliTime, cacheKey, isDegradedInput } = await getKundliWithCache(input);
    console.log(`[generateFullLifeReport] Kundli data ready (${kundliTime}ms), isDegradedInput: ${isDegradedInput || false}`);
    ...
    let prompt = generateFullLifePrompt(
      {
        name: input.name,
        dob: input.dob,
        tob: input.tob,
    ...
    if (isDegradedInput) {
      prompt = addDegradationAwarenessToPrompt(prompt, input);
      console.warn(`[generateFullLifeReport] ⚠️ DEGRADED INPUT - Added degradation awareness to prompt`);
    }

    // Generate AI content (pass reportType for complex report handling)
    console.log(`[generateFullLifeReport] Generating AI content${isDegradedInput ? ' (DEGRADED INPUT MODE)' : ''}...`);
    const aiStartTime = Date.now();
...
export async function generateYearAnalysisReport(
  ...
  try {
    ...
    // Get astrology data with caching (optimized)
    const { kundli: kundliResult, kundliTime, isDegradedInput } = await getKundliWithCache(input);
    console.log(`[generateYearAnalysisReport] Kundli data ready (${kundliTime}ms), isDegradedInput: ${isDegradedInput || false}, extracting planetary data...`);
    ...
    let prompt = generateYearAnalysisPrompt(
      {
        name: input.name,
        dob: input.dob,
        tob: input.tob,
 ...
 */
export async function generateMajorLifePhaseReport(input: AIAstrologyInput, sessionKey?: string, isRetry?: boolean): Promise<ReportContent> {
  const startTime = Date.now();
  try {
    console.log(`[generateMajorLifePhaseReport] Starting report generation for ${input.name}`);
    // Get astrology data with caching (optimized)
    const { kundli: kundliResult, kundliTime, isDegradedInput } = await getKundliWithCache(input);
    console.log(`[generateMajorLifePhaseReport] Kundli data ready (${kundliTime}ms), isDegradedInput: ${isDegradedInput || false}, extracting planetary data...`);
    ...
    let prompt = generateMajorLifePhasePrompt(
      {
        name: input.name,
        dob: input.dob,
        tob: input.tob,
    ...
    // CRITICAL FIX (Vercel Logs Analysis): Add stricter instructions for retry to prevent placeholder content
    if (isRetry) {
      const retryInstructions = `

CRITICAL RETRY INSTRUCTIONS (THIS IS A RETRY ATTEMPT):
- The previous attempt contained placeholder content or generic text
- You MUST generate REAL, PERSONALIZED content based on the birth details provided
- DO NOT use placeholder text like "Lorem ipsum", "[Name]", "[Date]", "TODO", "FIXME", or generic templates
- DO NOT use example text or sample content
- EVERY section must be personalized to ${input.name} (born ${input.dob})
- Reference specific planetary positions, signs, and houses from the provided data
- Use the actual date windows and years provided in the prompt
- Generate COMPLETE, DETAILED content - minimum word counts are mandatory
- If you cannot generate personalized content, you must still avoid placeholder text at all costs
- This is a RETRY - quality and personalization are CRITICAL`;
      prompt = prompt + retryInstructions;
      console.log(`[generateMajorLifePhaseReport] RETRY MODE - Added stricter placeholder prevention instructions`);
    }

    // Generate AI content (pass reportType for complex report handling)
    console.log(`[generateMajorLifePhaseReport] Generating AI content${isRetry ? ' (RETRY MODE)' : ''}${isDegradedInput ? ' (DEGRADED INPUT MODE)' : ''}...`);
    const aiStartTime = Date.now();
...
export async function generateDecisionSupportReport(
  ...
  try {
    ...
    // Get astrology data with caching (optimized)
    const { kundli: kundliResult, kundliTime, isDegradedInput } = await getKundliWithCache(input);
    console.log(`[generateDecisionSupportReport] Kundli data ready (${kundliTime}ms), isDegradedInput: ${isDegradedInput || false}, extracting planetary data...`);
    ...
    // Generate prompt
    let prompt = generateDecisionSupportPrompt(
      {
        name: input.name,
        dob: input.dob,
        tob: input.tob,
    ...
    if (isDegradedInput) {
      prompt = addDegradationAwarenessToPrompt(prompt, input);
      console.warn(`[generateDecisionSupportReport] ⚠️ DEGRADED INPUT - Added degradation awareness to prompt`);
    }
...
CRITICAL RETRY INSTRUCTIONS (THIS IS A RETRY ATTEMPT):
- The previous attempt contained placeholder content or generic text
- You MUST generate REAL, PERSONALIZED content based on the birth details and decision context provided
- DO NOT use placeholder text like "Lorem ipsum", "[Name]", "[Date]", "TODO", "FIXME", or generic templates
- DO NOT use example text or sample content
- EVERY section must be personalized to ${input.name} (born ${input.dob})
- Reference specific planetary positions, signs, and houses from the provided data
- If decision context was provided, integrate it meaningfully into the guidance
- Generate COMPLETE, DETAILED content - minimum word counts are mandatory
- If you cannot generate personalized content, you must still avoid placeholder text at all costs
- This is a RETRY - quality and personalization are CRITICAL`;
      prompt = prompt + retryInstructions;
      console.log(`[generateDecisionSupportReport] RETRY MODE - Added stricter placeholder prevention instructions`);
    }

    // Generate AI content (pass reportType for proper retry handling and logging)
    console.log(`[generateDecisionSupportReport] Generating AI content${isRetry ? ' (RETRY MODE)' : ''}${isDegradedInput ? ' (DEGRADED INPUT MODE)' : ''}...`);
    const aiStartTime = Date.now();
 ...
 * Adds a subtle disclaimer section at the beginning of the report when quality is LOW
 * This improves transparency and matches ChatGPT's recommendation
 */
export function addLowQualityDisclaimer(content: ReportContent): ReportContent {
  if (content.quality !== "LOW") {
    return content;
  }

  // Check if disclaimer already exists (avoid duplicates)
  const hasDisclaimer = content.sections?.some(
    section => section.title?.toLowerCase().includes("note") || 
               section.title?.toLowerCase().includes("disclaimer") ||
               section.content?.includes("concise mode")
  );
  ...
  const disclaimerSection: ReportSection = {
    title: "Note",
    content: "This report was generated in concise mode for faster delivery. While all key insights are included, some sections may be more condensed than usual.",
  };

  return {
    ...content,
    sections: [disclaimerSection, ...(content.sections || [])],
  };
}
