...
export type StoredReportStatus = "processing" | "completed" | "DELIVERED" | "failed"; // "completed" kept for backward compatibility, "DELIVERED" is the new standard
...
export type ReportErrorCode =
  ...
  | "STALE_PROCESSING" // Report stuck in processing > 5 minutes (new)
  | "UNKNOWN_ERROR";
...
/**
 * Get stale processing reports (stuck in processing > threshold minutes)
 * CRITICAL: Used by background job to auto-refund stuck reports
 * 
 * @param thresholdMinutes - Minutes since last update to consider stale (default: 5)
 * @returns Array of stale reports that need processing
 */
export async function getStaleProcessingReports(thresholdMinutes: number = 5): Promise<StoredReportRow[]> {
  if (!isSupabaseConfigured()) return [];
  const supabase = createServerClient();
  
  // Calculate threshold time (now - thresholdMinutes)
  ...
  const { data, error } = await supabase
    ...
    .lt("updated_at", thresholdISO) // updated_at is older than threshold
...
/**
 * Fetch reports that are in "processing" but have no content yet.
 * Used by background workers to pick up async-heavy reports.
 */
export async function getProcessingReportsWithoutContent(params?: {
  ...
  const olderThanSeconds = params?.olderThanSeconds ?? 10;
  const limit = params?.limit ?? 1;

  const thresholdTime = new Date(Date.now() - olderThanSeconds * 1000).toISOString();
  ...
  if (params?.reportId) {
    q = q.eq("report_id", params.reportId);
  }
