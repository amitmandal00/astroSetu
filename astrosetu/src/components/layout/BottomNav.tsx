      ...
      <div className="grid grid-cols-4 h-16 max-w-md mx-auto">
        {bottomNavItems.map((item) => {
          ...
          return (
            ...
            >
              <div className={clsx(
                "absolute inset-0 rounded-t-2xl transition-all",
                isActive ? "bg-gradient-to-b from-saffron-100 to-transparent" : ""
              )} />
              <span className={clsx(
                "text-2xl relative z-10 transition-transform",
                isActive && "scale-110"
              )}>{item.icon}</span>
              <span className={clsx(
              ...
              {isActive && (
                <div className="absolute top-0 left-1/2 -translate-x-1/2 w-12 h-1 bg-gradient-to-r from-saffron-500 via-amber-500 to-saffron-500 rounded-b-full" />
              )}
            </Link>
          );
        })}
