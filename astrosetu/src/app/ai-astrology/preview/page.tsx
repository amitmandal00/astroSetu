...
function PreviewContent() {
  ...
  // This must match the exact condition used to render the generation UI (line 2305)
  // Timer must match UI visibility, not just loading state
  // CRITICAL FIX: Use session_id (not sessionId) and remove reportType from processing conditions
  const isProcessingUI = useMemo(() => {
    ...
    // CRITICAL FIX: Match exact loader visibility condition (after removing reportType)
    return loading || isGeneratingRef.current || bundleGenerating || loadingStage !== null || shouldWaitForProcess || controllerActive;
  }, [loading, bundleGenerating, loadingStage, searchParams, bundleType, bundleReports.length, generationController.status]);
  ...
  const retryBundleGeneration = useCallback(async (
    ...
    hasAutoGeneratedRef.current = false;
    setBundleGenerating(false);
    // Clear any "redirected" refs if they block restart
