...
function PreviewContent() {
  ...
  const generateReport = useCallback(async (inputData: AIAstrologyInput, type: ReportType, currentSessionId?: string, currentPaymentIntentId?: string) => {
    ...
    if (loadingStartTimeRef.current === null) {
      ...
      // CRITICAL FIX: Don't set elapsedTime to 0 here - let the useEffect calculate it immediately
      // Setting it to 0 causes a flash of 0s before the useEffect runs
      // The useEffect will calculate the correct elapsed time (which will be 0 for a new timer, but calculated correctly)
    }
                ...
                if (savedInput) {
                  ...
                  requestAnimationFrame(() => {
                    if (!isGeneratingRef.current && !hasAutoGeneratedRef.current) {
                      ...
                      loadingStartTimeRef.current = startTime;
                      setLoadingStartTime(startTime);
                      // CRITICAL FIX: Don't set elapsedTime to 0 here - let the useEffect calculate it
                      // This prevents the timer from showing 0s when auto-generation starts
                      generateReport(inputData, "life-summary", undefined, undefined).catch((error) => {
                        console.error("[CLIENT] Error generating report for stale free report:", error);
                        hasAutoGeneratedRef.current = false;
                        setLoading(false);
          ...
          if (isFreeLifeSummary) {
            ...
            requestAnimationFrame(() => {
              try {
                ...
                console.log("[Preview] Executing generateReport for free life-summary");
                generateReport(inputData, reportTypeToUse, undefined, undefined).catch((error) => {
                  console.error("[Preview] Error in generateReport for free life-summary:", error);
                  hasAutoGeneratedRef.current = false; // Reset guard on error
                  setLoading(false);
                  setError(error.message || "Failed to generate report. Please try again.");
                });
            ...
            // For paid reports: Use setTimeout with delay to ensure payment verification state is set
            setTimeout(() => {
              ...
              if (isBundle && savedBundleReports) {
                try {
                  ...
                  const bundleReportsList = JSON.parse(savedBundleReports) as ReportType[];
                  console.log("[Preview] Generating bundle reports:", bundleReportsList);
                  generateBundleReports(inputData, bundleReportsList, urlSessionId || undefined, paymentIntentIdFromStorage);
                } catch (e) {
                  console.error("Failed to parse bundle reports:", e);
                  hasAutoGeneratedRef.current = false; // Reset guard on error
                  // Fallback: if bundle parsing fails and we have a reportTypeToUse, try single report
                  if (reportTypeToUse) {
                ...
                // For paid reports, pass session_id and paymentIntentId if available
                ...
                console.log("[Preview] Calling generateReport with delay for paid report");
  ...
  useEffect(() => {
    // CRITICAL FIX: Always sync refs with state to ensure they're current
    // This must happen on every render to keep refs in sync
    ...
    if (loading) {
      ...
      // CRITICAL FIX: Recreate interval when reportType or bundleGenerating changes
      ...
      const interval = setInterval(() => {
        ...
        // CRITICAL FIX: Use refs instead of state to avoid closure issues
        // Refs are synced at the start of each interval tick to ensure current values
  ...
  // the timer displays the correct value immediately, even if the interval was recreated.
  ...
  const shouldWaitForProcessForLoading = loading || isGeneratingRef.current || urlHasReportTypeForLoading || urlSessionIdForLoadingCheck || urlReportIdForLoadingCheck || autoGenerateForLoading || hasBundleInfoForLoading;
  const isWaitingForStateForLoading = (urlHasReportTypeForLoading || hasBundleInfoForLoading) && !input && !hasRedirectedRef.current && !loading;
  ...
  if (loading || isGeneratingRef.current || shouldWaitForProcessForLoading || isWaitingForStateForLoading) {
    ...
    const isInitializing = isWaitingForStateForLoading && !loading && !isGeneratingRef.current; // Initial state before generation starts
  ...
  if (!reportContent || !input) {
    ...
    if (loading || isGeneratingRef.current || shouldWaitForProcess || isWaitingForState) {
