...
function PreviewContent() {
  ...
  useEffect(() => {
    ...
    // Also check if we're on pre-loading screen (shouldWaitForProcess true) but generation hasn't started - this catches the looping case
    const isOnPreLoadingScreen = reportType === "life-summary" && (shouldWaitForProcess || isWaitingForState) && !loading && !reportContent;
    const shouldHaveGenerated = (reportType === "life-summary" && input && !loading && !reportContent && !isGeneratingRef.current) || isOnPreLoadingScreen;
    ...
    if (isStuckWithGuard || isStuckWithoutGuard) {
      ...
      stuckTimeout = setTimeout(() => {
        ...
        if (stillStuck) {
          ...
          // Use requestAnimationFrame for immediate execution (faster than setTimeout)
          requestAnimationFrame(() => {
  ...
  }, [reportType, input, loading, reportContent, searchParams, generateReport, bundleReports.length, bundleType]);
  ...
  if (!reportContent || !input) {
    ...
    if (shouldWaitForProcess || isWaitingForState) {
      ...
      // CRITICAL: Don't treat bundles as life-summary even if reportType is "life-summary"
      const isLifeSummary = reportType === "life-summary" && !hasBundleInfo;
          ...
          <Card className="max-w-2xl w-full mx-4">
            <CardContent className="p-12 text-center">
              ...
              {isLifeSummary ? (
                ...
                <>
                  {/* Enhanced copy for paid reports with time context and anti-refresh protection */}
                  <p className="text-slate-600 mb-4">
                    {urlHasReportType 
                      ? `Your personalized ${reportName.toLowerCase()} report is being prepared. Most reports are ready in 30–60 seconds.` 
                      : "Your personalized astrology report is being prepared. Most reports are ready in 30–60 seconds."}
                    <br />
                    <span className="text-sm font-medium">Please don&apos;t refresh — your report is being generated securely.</span>
                  </p>
                  
                  {/* Progress indicator for paid reports */}
                  <div className="mt-6 mb-6 space-y-3 text-left bg-slate-50 rounded-lg p-4 border border-slate-200">
                    <div className="flex items-center gap-2 text-sm text-purple-600">
                      <span className="font-bold">⏳</span>
                      <span className="font-medium">Preparing your personalized report...</span>
                    </div>
                  </div>
                  
                  {/* Value reinforcement for paid reports */}
                  <div className="mt-6 pt-6 border-t border-slate-200">
                    <p className="text-xs font-semibold text-slate-700 mb-3">You&apos;ll receive:</p>
                    <div className="space-y-2 text-left bg-gradient-to-r from-purple-50 to-indigo-50 rounded-lg p-4 border border-purple-200">
                      {getReportBenefits(reportType).map((benefit, idx) => (
