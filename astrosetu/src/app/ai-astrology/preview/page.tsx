...
function PreviewContent() {
  ...
  useEffect(() => {
    if (loading) {
      ...
      const interval = setInterval(() => {
        ...
        // For bundle reports, use longer timeout (100s) since multiple reports are being generated
        // Note: reportType is captured in closure, but we check bundle state via bundleGenerating state
  ...
  }, [loading, loadingStage]); // Removed reportType and loadingStartTime to prevent unnecessary interval recreation
  // Note: reportType is intentionally removed from dependencies because:
  // 1. It causes the interval to be recreated when reportType changes (e.g., during bundle generation)
  // 2. This can cause the timer to stop or reset unexpectedly
  // 3. The timeout threshold calculation uses reportType from closure, which is acceptable
  // 4. For bundle reports, we don't want the timer to restart when reportType changes
