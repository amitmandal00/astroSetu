...
function PreviewContent() {
  ...
  useEffect(() => {
    ...
    // Add a delay to avoid false positives (give the main useEffect time to trigger generation first)
    if (isLifeSummaryStuck) {
      stuckTimeout = setTimeout(() => {
        // Double-check we're still stuck before retrying
        if (reportType === "life-summary" && input && !loading && !reportContent && !isGeneratingRef.current && hasAutoGeneratedRef.current) {
          console.warn("[Life Summary] Detected stuck state after delay - resetting hasAutoGeneratedRef and triggering generation");
          hasAutoGeneratedRef.current = false; // Reset to allow retry
          setLoading(true);
          setLoadingStage("generating");
          setLoadingStartTime(Date.now());
      ...
      }, 2000); // Wait 2 seconds before detecting stuck state (gives main useEffect time to trigger)
    }
