...
export async function GET(req: Request) {
  ...
  try {
    ...
    // MVP SAFETY: Never process prodtest_ sessions in production without explicit flag
    ...
    // CRITICAL FIX: Always recognize prodtest_ sessions as test sessions (if allowed)
    // If a prodtest_ session exists and is allowed, it was created by create-checkout as a mock session
    // We must handle it as a test session, but only if ALLOW_PROD_TEST_BYPASS is enabled in production
