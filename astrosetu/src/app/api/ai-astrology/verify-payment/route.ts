...
export async function GET(req: Request) {
  ...
  try {
    ...
    // P0.2: HARD-BLOCK PRODTEST SESSIONS IN PRODUCTION
    // MVP SAFETY: In production, NEVER process prodtest_ sessions unless explicitly enabled
    ...
    // P0.2: In production, hard-block prodtest_ sessions unless explicitly enabled
    if (isProdTestSession && isProd && !allowProdTestBypass) {
      ...
      console.error("[PRODTEST BLOCKED - VERIFY PAYMENT]", JSON.stringify(prodtestBlockError, null, 2));
    ...
    // P0.2: Determine if this is a test session
    // In production: Only recognize prodtest_ if explicitly enabled
    // In non-production: Always recognize test sessions
    const isTestSession = isDemoTestSession || (isProdTestSession && (allowProdTestBypass || !isProd));
