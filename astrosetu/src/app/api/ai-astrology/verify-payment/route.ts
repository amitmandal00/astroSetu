...
export async function GET(req: Request) {
  ...
  try {
    ...
    // P0 FIX #2: Recognize prodtest_ sessions as test sessions
    // MVP SAFETY: prodtest_ sessions are created by create-checkout for test users
    // If a prodtest_ session exists, it means create-checkout already validated the user as a test user
    // So we always recognize prodtest_ sessions as test sessions (they're already validated)
    // The protection is in create-checkout (which blocks non-test users from creating prodtest_ sessions)
    ...
    // CRITICAL FIX: Always recognize prodtest_ sessions as test sessions
    // If a prodtest_ session exists, it was created by create-checkout for a test user
    // create-checkout already validated the user, so we trust the session
    // test_session_ always bypasses (demo mode)
    // prodtest_ always bypasses (created by test users, validated in create-checkout)
    ...
    try {
      ...
      // resource_missing is an expected error for invalid/missing sessions
      // This should be a 400 (client error) not 500 (server error)
      ...
      // For unexpected Stripe errors, return 500
      return NextResponse.json(
