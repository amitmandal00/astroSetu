...
import { getYogas } from "@/lib/prokeralaEnhanced";
...
/**
 * POST /api/astrology/yoga
 * Get detailed Yoga analysis (Raj Yogas, Dhan Yogas, etc.)
 */
export async function POST(req: Request) {
  ...
  try {
    // Rate limiting
    const rateLimitResponse = checkRateLimit(req, '/api/astrology/yoga');
    ...
    const result = await getYogas(input);
    
    return NextResponse.json(
      { ok: true, data: { yogas: result || [] }, requestId },
      {
        headers: {
