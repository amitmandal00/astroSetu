...
import { getPapaDosham } from "@/lib/prokeralaEnhanced";
...
export async function POST(req: Request) {
  ...
  try {
    // Rate limiting
    const rateLimitResponse = checkRateLimit(req, '/api/astrology/papa-dosha');
    ...
    const result = await getPapaDosham(input);
    
    if (!result) {
      return NextResponse.json(
        { ok: false, error: "Papa Dosha analysis not available" },
        { status: 503 }
      );
    }
