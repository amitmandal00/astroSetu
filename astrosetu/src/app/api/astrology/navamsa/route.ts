...
import { getNavamsaChart } from "@/lib/prokeralaEnhanced";
 ...
 * Get Navamsa Chart (D9 Chart) - Divisional chart for marriage and relationships
 */
export async function POST(req: Request) {
  ...
  try {
    // Rate limiting
    const rateLimitResponse = checkRateLimit(req, '/api/astrology/navamsa');
    ...
    const result = await getNavamsaChart(input);
    
    if (!result) {
      return NextResponse.json(
        { ok: false, error: "Navamsa chart not available" },
        { status: 503 }
      );
    }
